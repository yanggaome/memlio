...
- Call `memlio_store` to save the original and the user's reason for saving. Use `kind: note` for literal notes resembling paths/URLs. For images, supply a factual description only when you can inspect the image. The server does not perform OCR or image understanding yet.
