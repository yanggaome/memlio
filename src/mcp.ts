...
export async function serve(home: string) {
  ...
  server.registerTool(
    ...
    {
      description:
        'Save user-selected text, a URL, or a local file. Preserves originals; URLs are fetched for a readable snapshot. Supply note for why it matters and description for image contents. Returns capture and indexing status.',
      inputSchema: {
    ...
    guarded(async (args) => {
      ...
      return summarize(await memory.store({ ...args, source: 'mcp' }, { roots }));
    }),
  );
  ...
  server.registerTool(
    ...
    guarded((args) => {
      ...
      return {
        ...
        original: item.kind === 'note' ? undefined : item.original,
