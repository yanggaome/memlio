...
export async function fetchPage(raw: string, redirects = 0, deadline = Date.now() + 20_000): Promise<{ url: string; body: string; contentType: string }> {
  ...
  const response = await new Promise<{ status: number; location?: string; body: string; contentType: string }>((resolve, reject) => {
    ...
    const req = transport.get(url, {
      ...
      if (status < 200 || status >= 300) { res.resume(); reject(new Error(`Page returned HTTP ${status}.`)); return; }
      const contentType = res.headers['content-type'] ?? '';
      if (!/^(text\/(html|plain)|application\/xhtml\+xml)/i.test(contentType)) {
        res.resume(); reject(new Error(`Unsupported page content type: ${contentType || 'unknown'}.`)); return;
      }
      let bytes = 0;
      const chunks: Buffer[] = [];
      res.on('data', (chunk: Buffer) => {
        bytes += chunk.length;
        if (bytes > MAX_BYTES) { req.destroy(new Error('Page exceeds the 5 MB capture limit.')); return; }
        chunks.push(chunk);
      });
      res.on('error', reject);
...
export function extractPage(html: string, url: string): { title: string; markdown: string } {
  ...
  if (!article?.content || !article.textContent?.trim()) throw new Error('No readable article content was found.');
