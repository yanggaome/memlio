...
test(
  'cached local model saves and retrieves a paraphrase across offline CLI processes',
  ...
  (t) => {
    ...
    const run = (...args) => {
      const result = spawnSync(process.execPath, [resolve('dist/cli.js'), '--home', home, '--json', ...args], {
        cwd: tmpdir(),
        encoding: 'utf8',
        env: { ...process.env, MEMLIO_OFFLINE: '1' },
        timeout: 60_000,
      });
    ...
    const retrieved = run('find', 'secure my online accounts');
    assert.equal(retrieved.mode, 'hybrid');
    assert.deepEqual(retrieved.warnings, []);
