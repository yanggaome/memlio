...
test('Codex setup preserves existing settings/comments and is idempotent',t=>{
  ...
  assert.match(text,/# keep me/);assert.equal(parsed.model,'custom');assert.equal(parsed.mcp_servers.other.command,'other');assert.equal(parsed.mcp_servers.memlio.command,process.execPath);
...
test('Claude setup merges settings and dry-run leaves files unchanged',t=>{
  ...
  setup('claude',join(root,'collection'),{targetHome:root,dryRun:true});assert.equal(JSON.parse(readFileSync(join(root,'.claude.json'),'utf8')).mcpServers.memlio,undefined);
  setup('claude',join(root,'collection'),{targetHome:root});const parsed=JSON.parse(readFileSync(join(root,'.claude.json'),'utf8'));
  assert.equal(parsed.theme,'dark');assert.equal(parsed.mcpServers.other.command,'other');assert.equal(parsed.mcpServers.memlio.type,'stdio');
});
test('setup refuses to replace an unrelated existing memlio server',t=>{
  const root=fixture(t);writeFileSync(join(root,'.claude.json'),JSON.stringify({mcpServers:{memlio:{command:'someone-else'}}}));
...
test('real MCP SDK client stores, searches and reads across fresh servers',async t=>{
  ...
  try {
    const tools=await first.listTools();assert.ok(tools.tools.some(t=>t.name==='memlio_store'));
    const response=await first.callTool({name:'memlio_store',arguments:{input:'A travel adapter for Japan',note:'Packing list'}});assert.ok(!response.isError);id=JSON.parse(response.content[0].text).id;
  }finally{await first.close();}
  const second=await connect();
  try {
    const response=await second.callTool({name:'memlio_search',arguments:{query:'packing'}});assert.equal(JSON.parse(response.content[0].text).results[0].id,id);
    const read=await second.callTool({name:'memlio_get',arguments:{id,maxChars:10}});assert.equal(JSON.parse(read.content[0].text).text.length,10);assert.equal(JSON.parse(read.content[0].text).nextOffset,10);
    const bad=await second.callTool({name:'memlio_store',arguments:{input:'/etc/hosts',kind:'file'}});assert.equal(bad.isError,true);
